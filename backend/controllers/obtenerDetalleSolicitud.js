...
async function obtenerDetalleSolicitud(req, res) {
  ...
  logger.info(`📄 Generar PDF Devolucion ID: ${id} | Iniciando consulta de detalle de solicitud...`);

  if (!id) {
    logger.warn(`📄 Generar PDF Devolucion ID: ${id} | ID de solicitud no proporcionado.`);
  ...
  try {
    ...
    if (solicitudResult.recordset.length === 0) {
      logger.warn(`📄 Generar PDF Devolucion ID: ${id} | Solicitud no encontrada.`);
      return res.status(404).json({ message: "Solicitud no encontrada." });
    }

    const solicitud = solicitudResult.recordset[0];
    logger.info(`📄 Generar PDF Devolucion ID: ${id} | Solicitud encontrada con ID real: ${solicitud.solicitudId}`);
