...
async function createDevolucion(req, res) {
  ...
  try {
    ...
    console.log("esta vrg: ", createdDevolucion[0]?.devolucionId);
    try {
      const destinatario = "lecmbogota@gmail.com";
      const spResponse = {
        SolicitudId: createdDevolucion[0]?.devolucionId, // Puedes cambiar esto si quieres usar otro identificador
